...
if len(sys.argv) < 3:
    print "This script is used to merge several XML blast outputs into a single file \nwhen queries are run in parallel by input-choping.\n"
    print "Usage: python " + sys.argv[0] + " output_xml input_xmls \n"
    exit()

# sub function
#-----------------------------------------------------------
def merge_blast_xml(split_files, output_file):
    ...
    for f in split_files:
        ...
        if header.strip() not in ['<?xml version="1.0"?>',
...
merge_blast_xml(xmlfiles,xmlout)
